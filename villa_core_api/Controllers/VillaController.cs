...
public class VillaController : ControllerBase
{
    ...
    public ActionResult<IEnumerable<VillaDto>> Get([FromQuery] int limit = 5, [FromQuery] int offset = 0)
    {
        var villas = _villaService.GetVillas(limit, offset);
        return Ok(villas);
    }
    ...
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public ActionResult<VillaDto> Post([FromBody] VillaCreateDto villaCreateDto)
    {
        ...
        if(_villaService.CheckVillaNameExist(villaCreateDto.Name)){
    ...
    public ActionResult DeleteById(int id)
    {
        ...
        var isVillaDeleted = _villaService.DeleteVilla(id);
        if(isVillaDeleted){
            return NoContent();
        }else{
            return NotFound();
    ...
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public ActionResult UpdateById([FromRoute] int id, [FromBody] VillaCreateDto villaCreateDto){
        ...
        var isVillaUpdated = _villaService.UpdatedVilla(id, villaCreateDto);
        if(isVillaUpdated){
            return NoContent();
        }else{
            return NotFound();
    ...
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public ActionResult UpdateDynamicFieldById([FromRoute] int id, [FromBody] JsonPatchDocument<VillaCreateDto> villaUpdateDto){
        if (id <= 0 || villaUpdateDto == null)
        {
            return BadRequest();
        }
        
        bool isVillaUpdated = _villaService.UpdatedDynamicFieldVilla(id, villaUpdateDto, ModelState);
